...
## ✅ slug 语义整治（2026-07-23，与主工程 E_BAD_SLUG 拒收联动）

- [x] `cleanEngineTitle`：index.html \<title\> 剥引擎样板前缀（Unity WebGL Player/团结引擎/Cocos Creator…），剥完只剩构建目录名（webgl/web-mobile/dist…）或空 → 本地拒发要真名，不浪费一次上传；黑名单与服务端 GENERIC_SLUGS 同步维护（2026-07-23）
- [x] 测试 15→17：样板剥壳矩阵 + 引擎默认标题本地拒发（2026-07-23）
